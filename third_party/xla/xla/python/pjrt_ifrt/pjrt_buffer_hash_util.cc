    ...
    pjrt_buffers[i]
        ...
        .OnReady([&, i, literal = *std::move(literal),
          ...
          tsl::Env::Default()->SchedClosure(
              [&, i, literal = std::move(literal),
               inflight_hashing = std::move(inflight_hashing)]() mutable {
                absl::StatusOr<uint64_t> hash =
                  ...
                  switch (mode) {
                    ...
                    case Client::HashMode::kLogical:
                      return HashBufferLogical(literal_span,
                                               element_byte_sizes[i],
                                               index_domains[i]);
                  }
                }();
