...
TEST(PjRtBufferHashUtilTest, HashPjRtBuffers) {
  ...
  TF_ASSERT_OK(pjrt_buffer->GetReadyFuture().Await());
  ...
  ASSERT_OK_AND_ASSIGN(
      ...
      HashPjRtBuffers(raw_buffers, index_domains, Client::HashMode::kLogical));
...
TEST(PjRtBufferHashUtilTest, HashPjRtBuffersSharding) {
  ...
  TF_ASSERT_OK(pjrt_buffer0->GetReadyFuture().Await());

  // Array 1: Shard 0 out of 2
  ...
  TF_ASSERT_OK(pjrt_buffer_shard0->GetReadyFuture().Await());

  // Array 2: Shard 1 out of 2
  ...
  TF_ASSERT_OK(pjrt_buffer_shard1->GetReadyFuture().Await());
